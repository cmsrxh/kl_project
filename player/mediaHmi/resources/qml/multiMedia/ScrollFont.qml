...
Item {
    ...
    onTextChanged:
    {
        console.log("Text: ", text, leftText.contentWidth, textLab.width)
        if( leftText.contentWidth > textLab.width )
        {
            animation.restart();
        } else
        {
            animation.stop()
            leftText.x = (textLab.width - leftText.contentWidth) / 2
        }
    }
    ...
    Text
    {
        ...
        SequentialAnimation on x {
            ...
            running: leftText.contentWidth > textLab.width
            PropertyAnimation {
                easing.type: Easing.Linear
                from: 0
                to: -leftText.contentWidth
                duration: leftText.contentWidth * 15
            }
            PropertyAnimation {
                easing.type: Easing.Linear
                from: textLab.width
                to: 0
                duration: textLab.width * 15
            }
        }
    }
}
