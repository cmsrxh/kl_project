...
void CurrentBackup::saveCurrent()
{
    ...
    if (fd > 0)
    {
        ...
        KLDataProc::instance()->getViewSwitchInfo(data, len);
        write(fd, data, len);
...
void CurrentBackup::recoveryCurrent()
{
    ...
    if (stat(LOCAL_KL_CURRENT_INFO_FILE, &st) == 0
            && st.st_size > (int)sizeof(KLDataProc::SwitchPath))
    {
        ...
        if (fd > 0)
        {
            ...
            {
                data[ret] = '\0';
                KLDataProc::instance()->setViewSwitchInfo(data);
                p = data + sizeof(KLDataProc::SwitchPath);
                if (0 == memcmp(p, CURRENT_MAGIC, sizeof(CURRENT_MAGIC)-1))
                {
                    p += sizeof(CURRENT_MAGIC)-1;
                    priserCurrentInfo(p, st.st_size - sizeof(KLDataProc::SwitchPath) - sizeof(CURRENT_MAGIC) + 1);
                }
            } else
            {
...
void CurrentBackup::playPrevRecordInfo()
{
            ...
            && mRecPlay.type > 0)
    {
        ...
        KLDataProc::instance()->localItemPlay(PLAY_CHIP_TYPE_PREV_PLAYING_RECORD, 0, m_pPrevRecChip);
    }
}

char* CurrentBackup::genCurrentInfo()
{
    const CollectNode *curPlay = KLDataProc::instance()->getPlayInfoIfPlaying();
    char *data = nullptr;
